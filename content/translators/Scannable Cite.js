...
const Mem = function (item) {
  ...
  this.set = function (str, punc, slug) {
    if (!punc) {
      punc = ''
    }
    if (str) {
...
function doExport() {
  ...
  while ((item = Zotero.nextItem())) {
    ...
    if (
      Zotero.getHiddenPref('ODFScan.includeTitle') ||
    ...
    if (Zotero.getHiddenPref('ODFScan.useZoteroSelect')) {
      ...
      let m = item.uri.match(
        /http:\/\/zotero\.org\/(users|groups)\/([^\/]+)\/items\/(.+)/,
      )
      let prefix
      let lib
      if (m) {
