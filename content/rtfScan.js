...
var Zotero_ODFScan = new (function () {
  ...
  let unmappedCitationsChildren,
    ambiguousCitationsChildren,
    mappedCitationsChildren
  let citations, citationItemIDs, allCitedItemIDs, contents

  // Load in the localization stringbundle for use by getString(name)
  let stringBundleService = Components.classes[
  ...
  function _getString(name, params) {
    ...
    try {
      if (params != undefined) {
        ...
        l10n = _localizedStringBundle.formatStringFromName(
          name,
          params,
          params.length,
        )
  ...
  this.introPageShowing = function () {
    ...
    let selectedNode = document.getElementById(
      'file-type-selector-' + mode_string,
    )
  ...
  this.introPageAdvanced = function () {
    ...
    let outputMode = Zotero.Prefs.get('ODFScan.outputMode')
    Zotero.Prefs.set(
      'ODFScan.' + fileType + '.lastInputFile' + outputMode,
      inputFile.path,
    )
    Zotero.Prefs.set(
      'ODFScan.' + fileType + '.lastOutputFile' + outputMode,
      outputFile.path,
    )
  }
  ...
  this.chooseInputFile = async function () {
    ...
    document
      .getElementById('odf-file-error-message')
    ...
    let outputMode = Zotero.Prefs.get('ODFScan.outputMode')
    let inputPath = Zotero.Prefs.get(
      'ODFScan.' + fileType + '.lastInputFile' + outputMode,
    )
    if (inputPath) {
      if (!inputFile) {
  ...
  this.chooseOutputFile = async function () {
    ...
    if (inputFile) {
      ...
      let suffix =
        ' ' +
        _getString('ODFScan.' + fileType + '.scannedFileSuffix' + outputMode)
      if (fileType === 'odf') {
        let suffixMatchers = [
          ' ' + _getString('ODFScan.odf.scannedFileSuffixtomarkers'),
          ' ' + _getString('ODFScan.odf.scannedFileSuffixtocitations'),
        ]
        for (let suffixMatcher of suffixMatchers) {
          if (
            leafName.slice(-suffixMatcher.length, leafName.length) ==
            suffixMatcher
          ) {
    ...
    let outputPath = Zotero.Prefs.get(
      'ODFScan.' + fileType + '.lastOutputFile' + outputMode,
    )
    if (outputPath) {
      if (!outputFile) {
  ...
  function _updatePath() {
    ...
    if (inputFile && inputFile.path) {
      ...
      document.getElementById('input-path').value = _getString(
        'ODFScan.file.noneSelected.label',
      )
    ...
    if (outputFile) {
      ...
      document.getElementById('output-path').value = _getString(
        'ODFScan.file.noneSelected.label',
      )
    }
  }
  ...
  function _scanODF(outputMode) {
    ...
    let tmplCitation =
      '<text:reference-mark-start text:name="ZOTERO_ITEM {&quot;properties&quot;:{&quot;formattedCitation&quot;:&quot;%{1}s&quot;},&quot;citationItems&quot;:%{2}s} RND%{3}s"/>%{4}s<text:reference-mark-end text:name="ZOTERO_ITEM {&quot;properties&quot;:{&quot;formattedCitation&quot;:&quot;%{5}s&quot;},&quot;citationItems&quot;:%{6}s} RND%{7}s"/>'
    let tmplText = '{ %{1}s | %{2}s | %{3}s | %{4}s |%{5}s}'

    let rexPref =
      /<meta:user-defined meta:name="ZOTERO_PREF[^<]*?<\/meta:user-defined>/
    let rexLabels =
      /^((?:art|ch|Ch|subch|col|fig|l|n|no|op|p|pp|para|subpara|pt|r|sec|subsec|Sec|sv|sch|tit|vrs|vol)\\.)\\s+(.*)/
    let rexBalancedTags =
      /(.*)<([-:a-zA-Z0-9]*)[^\/>]*>([^<]*)<\/([-:a-zA-Z0-9]*)[^>]*>(.*)/
    let rexLink =
      /(<[^>]*xlink:href=\"([^\"]*)\"[^>]*>)\s*{([^\|{}]*)\|([^\|}]*)\|([^\|}]*)\|([^\|}]*)}\s*(<[^>]*>)/
    let rexLink2 =
      /(<[^>]*xlink:href=\"([^\"]*)\"[^>]*>)\s*(?:<[^\/>]+>)\s*{([^\|{}]*)\|([^\|}]*)\|([^\|}]*)\|([^\|}]*)}\s*(?:<\/[^\/>]+>)\s*(<[^>]*>)/
    let rexNativeLink =
      /<text:reference-mark-start[^>]*ZOTERO_ITEM\s+(?:CSL_CITATION\s+)*([^>]*)\s+[^ ]*\/>(.*?)<text:reference-mark-end[^>]*\/>/
    let checkStringRex =
      /(<[^\/>][^>]*>)*{[^<>\|]*|[^<>\|]*|[^<>\|]*|[^<>\|]*|[^<>\|]*}(<\/[^>]*>)*/
    let openTagSplitter = /(<[^\/>][^>]*>)/
    ...
    let rexWrappedLinks =
      /(<[^>]*xlink:href=\"[^\"]*\"[^>]*>\s*(?:<[^\/>]+>)?\s*{[^\|{}]*\|[^\|}]*\|[^\|}]*\|[^\|}]*}\s*(?:<\/[^\/>]+>)?\s*<[^>]*>)/
    let rexNativeLinks =
      /(<text:reference-mark-start[^>]*ZOTERO_ITEM\s+(?:CSL_CITATION\s+)*[^>]*\/>.*?<text:reference-mark-end[^>]*\/>)/
    let rexCite = /({[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*})/
    let rexCiteExtended =
      /(<\/?text:span[^>]*>{[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*}<\/?text:span[^>]*>)/
    let rexCiteExtendedParts =
      /(<text:span[^>]*>)({[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*})(<\/text:span>)/
    let rexCiteExtendedPartsReverse =
      /(<\/text:span>)({[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*})(<text:span[^>]*>)/
        ...
        if (m) {
          if (m[2] === m[4]) {
            ...
            throw (
              ...
              ' ' +
              m[4] +
              '. Original document is corrupt. Aborting.'
            )
          }
        } else {
          break
    ...
    Fragment.prototype.normalizeNativeMarks = function () {
      ...
      if (m) {
        ...
        for (let i = 0, ilen = obj.citationItems.length; i < ilen; i += 1) {
          ...
          if (
            (item.uri && item.uri.length) ||
            (item.uris && item.uris.length)
          ) {
            ...
            if (m_uri) {
              if (m_uri[1] === 'users') {
                ...
                if (
                  m_uri[2].includes('local') ||
                  Zotero.Prefs.get('translators.ODFScan.useZoteroSelect')
                ) {
                  key.push('0')
                } else {
                  key.push(m_uri[2])
          ...
          replacement += tmplText
            .replace('%{1}s', item.prefix)
    ...
    ODFConv.prototype.composeCitations = function () {
      ...
      for (let i = m.length - 1; i > -1; i += -1) {
        ...
        let link = this.fixMarkup(m[i][4])
          .replace(/^\s+/, '')
          .replace(/\s+$/, '')

        item.prefix = this.fixMarkup(m[i][0]).replace(/^\s+/, '')
        item.locator = this.fixMarkup(m[i][2])
          .replace(/^\s+/, '')
          .replace(/\s+$/, '')
        ...
        if (myidlst.length === 2) {
          ...
          if (params.isUserItem) {
            ...
            item.uri = [
              'http://zotero.org/users/' + userID + '/items/' + myidlst[1],
            ]
            ...
            item.uri = [
              'http://zotero.org/groups/' + groupID + '/items/' + myidlst[1],
            ]
            item.uris = item.uri.slice()
          }
        } else {
        ...
        if (lst[i]) {
          ...
          let citation = tmplCitation
            .replace('%{1}s', escapedPlaceholder)
            .replace('%{2}s', items)
            .replace('%{3}s', randstr)
    ...
    ODFConv.prototype.generateRandomString = function (str) {
      ...
      while (true) {
        let nums = [
          49, 50, 51, 52, 53, 54, 55, 56, 57, 65, 66, 67, 68, 69, 70, 71, 72,
    ...
    ODFConv.prototype.readZipfileContent = function () {
      ...
      function _getReader() {
        let zipReader = Components.classes[
    ...
    ODFConv.prototype.writeZipfileContent = function () {
      ...
      function _getWriter() {
        let zipWriter = Components.classes[
    ...
    ODFConv.prototype.purgeStyles = function () {
      let decodeXML = Components.classes[
  ...
  function _matchesItemCreators(creators, item, etAl) {
    ...
    let primaryCreators = []
    let primaryCreatorTypeID = Zotero.CreatorTypes.getPrimaryIDForType(
      item.itemTypeID,
    )
    ...
    if (
      itemCreators.length == creators.length ||
      (etAl && itemCreators.length > creators.length)
    ) {
      let matched = true
  ...
  function _matchesItemCreator(creator, itemCreator) {
    ...
    if (lowerLast != creator.substr(-lowerLast.length).toLowerCase())
      return false

    // make sure first name matches, if it exists
    if (creator.length > lowerLast.length) {
      let firstName = Zotero.Utilities.trim(
        creator.substr(0, creator.length - lowerLast.length),
      )
      if (firstName.length) {
        ...
        if (m) {
          ...
          let itemInitials = itemCreator.ref.firstName
            .split(/ +/g)
            .map(function (name) {
          ...
          let firstWord = firstName
            .substr(0, itemCreator.ref.firstName)
            .toLowerCase()
          let itemFirstWord = itemCreator.ref.firstName
            .substr(0, itemCreator.ref.firstName.indexOf(' '))
            .toLowerCase()
  ...
  this.citationsPageRewound = function () {
    ...
    while (unmappedCitationsChildren.hasChildNodes()) {
      unmappedCitationsChildren.removeChild(
        unmappedCitationsChildren.firstChild,
      )
    }
    while (ambiguousCitationsChildren.hasChildNodes()) {
      ambiguousCitationsChildren.removeChild(
        ambiguousCitationsChildren.firstChild,
      )
    }
    ...
    // hide headings
    unmappedCitationsItem.hidden =
      ambiguousCitationsItem.hidden =
      mappedCitationsItem.hidden =
        true
  ...
  this.treeClick = function (event) {
    ...
    if (col.value.index == 2 && level > 0) {
      ...
      if (level == 2) {
        ...
        // update array
        citationItemIDs[citation] = [
          citationItemIDs[citation][row - parentIndex - 1],
        ]
      } else {
        // mapped or unmapped citation, or ambiguous citation parent
        let citation = tree.view.getCellText(row, citationColumn)
        let io = { singleSelection: true }
        if (
          citationItemIDs[citation] &&
          citationItemIDs[citation].length == 1
        ) {
          // mapped citation
  ...
  this.stylePageAdvanced = function () {
    if (Zotero.Prefs.get('ODFScan.fileType') === 'rtf') {
      Zotero.Prefs.set(
        'export.lastStyle',
        document.getElementById('style-listbox').selectedItem.value,
      )
    }
  }
  ...
  function _formatRTF() {
    ...
    for (let i = 0; i < citations.length; i++) {
      ...
      if (
        isNote &&
        citations[i].end < contents.length &&
        '.,!?'.indexOf(contents[citations[i].end]) !== -1
      ) {
      ...
      if (isNote) {
        if (document.getElementById('displayAs').selectedIndex) {
          // endnotes
          contentArray.push(
            '{\\super\\chftn}\\ftnbj {\\footnote\\ftnalt {\\super\\chftn } ' +
              citation +
              '}',
          )
        } else {
          // footnotes
          contentArray.push(
            '{\\super\\chftn}\\ftnbj {\\footnote {\\super\\chftn } ' +
              citation +
              '}',
          )
        }
    ...
    if (zStyle.hasBibliography) {
      ...
      if (contents.indexOf(BIBLIOGRAPHY_PLACEHOLDER) !== -1) {
        ...
        bibliography =
          linebreak + '\\' + linebreak + '\\' + linebreak + bibliography
